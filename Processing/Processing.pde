...
final float reg_ac = 2, // Faixa do acelerometro: +/- 2g
            ...
            reg_pos = 1/9.81, // Faixa do deslocamento: +/- 100 cm
            reg_vel = 0.2/9.81, // Faixa de velocidade: +/- 20 cm/s
            reg_ajerk = 400,
            reg_gjerk = 1.0e+7; 
...
void draw() // Rotina em repeticao permanente
{
  ...
  scx = getreg(int(valorx.getValue())); // Selecao de variavel eixo X
  scy = getreg(int(valory.getValue())); // Selecao de variavel eixo Y
  scz = getreg(int(valorz.getValue())); // Selecao de variavel eixo Z
  ...
  for(j=0;j<c;j++)
    if(abs((1 + mode_scale)*f_dadoy[j]/scy) <= 1.0)
  ...
  {
    for(j=0;j<c;j++)
      if(abs((1 + mode_scale)*f_dadox[j]/scx) <= 1.0)
  ...
  else
  {
    ...
    for(j=0;j<c;j++)
      if(abs((1 + mode_scale)*f_dadoz[j]/scz) <= 1.0)
